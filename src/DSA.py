        ...
        for t in range(1, self.tmax + 1):
            ...
            for i, particle in enumerate(self.particles):
                if self.P > np.random.random():
                    gaussian_noise = np.random.normal(0, 1, self.dimension)  # 添加高斯随机噪声
                    particle.x = particle.x + Miu * particle.x * np.sign(np.random.random() - 0.5) + gaussian_noise  # 将高斯随机噪声加入到位置更新中
                else:
                    remain = list(indice)
                    remain.remove(i)
