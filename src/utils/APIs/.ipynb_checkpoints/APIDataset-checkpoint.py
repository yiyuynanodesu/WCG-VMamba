...
class APIDataset(Dataset):
    ...
    def __init__(self, texts, imgs, labels) -> None:
        self.texts = texts
        self.imgs = imgs
        self.labels = labels
    ...
    def __getitem__(self, index):
        return self.texts[index], self.imgs[index], self.labels[index]
    
    # collate_fn = None
    def collate_fn(self, batch):
        texts = [torch.LongTensor(b[0]) for b in batch]
        imgs = torch.FloatTensor([np.array(b[1]).tolist() for b in batch]) 
        ...
        return paded_texts, paded_texts_mask, imgs, labels